...
mape.manual.arima <- default.matrix
...
for(i in seq(start, end, by=1/12)) {
  ...
  fit.holt.winters <- HoltWinters(train)
  forecast.holt.winters <- forecast(fit.holt.winters)
  
  mape.manual.arima[i,1:length(test)] <- mape(test, forecast.manual.arima[['mean']])
...
mape.horizon <- data.frame(
  ...
  manual=colMeans(mape.manual.arima, na.rm = TRUE),
...
mape.training <- data.frame(
  index=seq(1,dim(mape.manual.arima)[1]),
  manual=rowMeans(mape.manual.arima, na.rm = TRUE),
  auto.arima=rowMeans(mape.auto.arima, na.rm = TRUE),
  ets=rowMeans(mape.ets, na.rm = TRUE),
  holt.winters=rowMeans(mape.holt.winters, na.rm = TRUE) 
)
...
ggplot(mape.training, aes(x=index)) +
  geom_line(aes(y=manual, color='manual')) +
  geom_line(aes(y=auto.arima, color='auto.arima')) +
  geom_line(aes(y=ets, color='ets')) +
  geom_line(aes(y=holt.winters, color='holt.winters'))
