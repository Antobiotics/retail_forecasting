...
mae.manual.arima <- matrix(NA,ceiling(end-start),12)
mae.auto.arima <- matrix(NA,ceiling(end-start),12)
mae.ets <- matrix(NA,ceiling(end-start),12)
...
#   forecast.manual.arima <- forecast(fit.manual.arima, h=12)
  ...
  forecast.auto.arima <- forecast(fit.arima, h=10)
  ...
  mae.auto.arima[i,1:length(test)] <- abs(forecast.auto.arima[['mean']]-test)
  # mae.ets[i,1:length(test)] <- abs(forecast.ets[['mean']]-test)
} 
